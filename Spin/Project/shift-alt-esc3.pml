...
*must instantiate three Customers (each 
*with a different favorite food), one 
*Cashier, and two Servers.
*/
      ...
      ::// Wait for an order
        ...
        atomic {
            ...
            if
              ...
              :: _pid % 2 == 1 -> Server2 = orders[myOrder].CustID;
            fi;
        }
...
/*
  ...
  4)It is always the case that eventually the server is busy 
    fulfilling an order.
  5)two servers never work on the same order
*/
...
//5)The server never work on the same order at the same time.
